<template>
  <div class="deptManager-content">
    ...
    <el-table :data="tableData" style="width: 100%;margin-top: 20px;">
      ...
      <el-table-column label="操作">
        <template slot-scope="scope">
          <el-button type="primary" size="small" @click="handleEditDevice(scope.row)" v-if="pageMenu.appslotset">设置
          </el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog title="管理应用" :visible.sync="dialogVisibleDevice" width="50%" :before-close="handleClose" append-to-body>
      <el-table :data="appInfo" style="width: 100%;margin-top: 20px;" max-height="700px">
        <el-table-column prop="name" label="设为底部">
          <template slot-scope="scope">
            <el-radio size="mini" v-model="editForm[scope.$index].Default" :label="1"
                      @change="changeCheckbox(scope.$index)">选择
            </el-radio>
          </template>
        </el-table-column>
        ...
        <el-table-column prop="name" label="置顶时间" width="500px">
          <template slot-scope="scope">
            <el-checkbox size="mini" v-for="(item,index) of timeInfo.timeRelateList"
                         v-show="editForm[scope.$index].Default == 0"
                         v-model="editForm[scope.$index].TimeSlot"
                         :label="item.timeSlotCode" :key="item.timeSlotCode">
              {{item.beginTimeSlot}}
            </el-checkbox>
          </template>
        </el-table-column>
