  ...
  <div class="deptManager-content">
    ...
    <el-form :inline="true" :model="search" class="demo-form-inline">
      <el-form-item label="关键字">
        <el-input v-model="search.SearchKey" style="width: 220px" placeholder="根据IP、设备名称、备注搜索"></el-input>
      </el-form-item>
    ...
    <!--  表格  -->
    <el-table :data="tableData" @selection-change="handleDeletion" style="width: 100%">
      ...
      <el-table-column prop="name" label="标签">
        <template slot-scope="scope">
          <img src="../../common/images/android.png" v-show="scope.row.systemType == 'Android'" width="25" alt="">
          <img src="../../common/images/win.png" v-show="scope.row.systemType == 'Windows'" width="25" alt="">
    ...
    <el-dialog title="关机时间" :visible.sync="dialogVisible" width="50%" :before-close="handleClose" append-to-body>
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="submitDownForm(tableChecked)">确 定</el-button>
      </span>
    </el-dialog>

    <!--  修改设备名  -->
    <el-dialog title="修改信息" :visible.sync="nameDialogVisible" width="50%" :before-close="handleClose" append-to-body>
...
<script>
	...
	export default {
		...
		methods: {
			...
			//同屏开关
			changeSyn(item) {
				let msg = '如果取消了同屏状态，设备将被移出原有的同屏组，会导致不能与其他的设备保持屏幕同步。'
				if (item.isSyn) {
					msg = '当设备设为同屏设备后，该设备无法被商家可见并且会移出现有的所在组。设备上已关联的排期也会被去除。'
				}
				this.$confirm(msg, "提示", {
					confirmButtonText: "确定",
					cancelButtonText: "取消",
