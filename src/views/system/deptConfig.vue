  ...
  <div class="deptManager-content">
    ...
    <el-form class="demo-form-inline">
      <el-form-item class="right-button">
        <el-button type="success" @click="handleEdit({})" v-show="pageMenu.addappkey">新增</el-button>
      </el-form-item>
    </el-form>
    ...
    <el-table :data="tableData" style="width: 100%;" height="620">
      ...
      <el-table-column label="锁定状态">
        <template slot-scope="scope">
          <el-switch v-model="scope.row.isLock" @change="changeShow(scope.row)"
                     :disabled="!pageMenu.lockappkey"></el-switch>
        </template>
      </el-table-column>
      <el-table-column label="操作">
        <template slot-scope="scope">
          <el-button type="primary" size="small" @click="handleWatch(scope.row)" v-show="pageMenu.getappkey">查看
          </el-button>
          <el-button type="danger" size="small" @click="handleDelete(scope.row)" v-show="pageMenu.delappkey">删除
          </el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog :title="dialogTitle" :visible.sync="dialogVisible" width="50%" :close-on-click-modal="false"
      ...
      <el-form :label-width="'120px'" :model="editForm" :rules="rules" ref="editForm">
        <el-form-item label="项目名称" prop="description">
          <el-input type="text" v-model="editForm.description" maxlength="20" style="width: 40%" placeholder="请输入项目名称"></el-input>
        </el-form-item>
      </el-form>
    ...
    <!--  查看  -->
    <el-dialog title="详情" :visible.sync="dialogVisibleWatch" width="50%" :before-close="handleClose" append-to-body>
      <div class="det-div">
      ...
      <div class="det-div">
        ...
        {{ watchForm.appKey }}
        <el-link type="success" class="copy-link" :underline="false" @click="clickCopy(watchForm.appKey)">复制</el-link>
        <p class="intro">开发识别码，配合开发者调用系统的接口能力</p>
      </div>
      <div class="det-div">
        <p class="title">开发者ID（AppSecret）:</p>
        {{ watchForm.secret }}
        <el-link type="success" class="copy-link" :underline="false" @click="clickCopy(watchForm.secret)">复制</el-link>
        <p class="intro">校验开发者身份的密码，具有极高的安全性。切记勿把密码直接交给第三方开发者或直接存储在代码中。</p>
      </div>
      <span slot="footer" class="dialog-footer">
        <el-button type="primary" @click="handleClose('editForm')">确 定</el-button>
      </span>
    </el-dialog>
  </div>
</template>
  ...
  export default {
    ...
    data() {
      return {
        ...
        deviceForm: {},
        rules: {
          description: [{required: true, message: '请输入项目名称', trigger: 'blur'}],
        },
        pageMenu: {},
        loading: false,
      }
