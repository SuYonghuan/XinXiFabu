  ...
  <div class="deptManager-content">
    ...
    <el-form :inline="true" :model="search" class="demo-form-inline">
      <el-form-item class="right-button">
        <el-button type="success" @click="handleEdit({})" v-if="pageMenu.addrole">新增角色</el-button>
        <el-button type="danger" @click="batchDelete(tableChecked)" v-if="pageMenu.delrole">删除</el-button>
      </el-form-item>
    </el-form>
    ...
    <el-table
      ...
      <el-table-column label="操作">
        <template slot-scope="scope">
          <el-button type="primary" size="small" @click="handleEdit(scope.row)" v-if="pageMenu.editrole" :disabled="scope.row.name == '超级管理员'">编辑</el-button>
          <el-button type="danger" size="small" @click="handleDelete(scope.row)" v-if="pageMenu.delrole" :disabled="scope.row.name == '超级管理员'">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
					...
					if (res.code === ERR_OK) {
						...
						for (let i = 0; i < this.treeData.length; i++) {
							for (let j = 0; j < this.treeData[i].childList.length; j++) {
								for (let k = 0; k < this.treeData[i].childList[j].actions.length; k++) {
									this.treeData[i].childList[j].actions[k].textCH = this.treeData[i].childList[j].actions[k].actionName
								}
								this.treeData[i].childList[j].childList = this.treeData[i].childList[j].actions
							}
						}
