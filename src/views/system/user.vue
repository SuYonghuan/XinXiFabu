    ...
    <el-form :inline="true" :model="search" class="demo-form-inline">
      <el-form-item class="right-button">
        <el-button
          type="success"
          @click="handleAdd({})"
          v-if="pageMenu.addaccount"
          >新增用户</el-button
        >
        <el-button
          type="danger"
          @click="batchDelete(tableChecked)"
          v-if="pageMenu.delaccount"
          >删除</el-button
        >
      </el-form-item>
    </el-form>
      ...
      <el-table-column label="操作" width="240px">
        <template slot-scope="scope">
          ...
          <el-button
            type="warning"
            size="small"
            @click="handleEditPsd(scope.row)"
            v-if="pageMenu.changepwd"
            :disabled="scope.row.roleName == '超级管理员'"
            >修改密码
          </el-button>
          <el-button
...
export default {
  ...
  data() {
    ...
    return {
      ...
      rules: {
        accountName: [
          { required: true, validator: UserRegular, trigger: "blur" },
        ],
        password: [{ required: true, validator: PassRegular, trigger: "blur" }],
        passagin: [
          { required: true, validator: AgainRegular, trigger: "blur" },
        ],
        roleCode: [
      ...
      pwdRules: {
        password: [{ required: true, validator: PassRegular, trigger: "blur" }],
        passagin: [
          { required: true, validator: AgainRegular2, trigger: "blur" },
        ],
      },
      tableChecked: [],
